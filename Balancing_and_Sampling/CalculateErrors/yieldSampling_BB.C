...
TTree* createTree(Int_t iTree) {
  ...
  TLegend* legendBpMonash = new TLegend(0.44, 0.49, 0.89, 0.89);
  legendBpMonash->SetTextSize(0.03);
  legendBpMonash->SetBorderSize(0);
  ...
  if (iTree == 0) { // BBMULTGENERAL
    ...
    BpMonashTemplate->SetTitle("");
    BpMonashTemplate->GetYaxis()->SetTitle("counts");
    BpMonashTemplate->GetXaxis()->SetTitle("balanced yield B^{+}B^{-}");
    BpMonashTemplate->GetYaxis()->SetRangeUser(0, 25);
    BpMonashTemplate->GetXaxis()->SetRangeUser(0.32, 0.52);
    BpMonashTemplate->Draw("hist");
  }
  
  if (iTree == 1) { // BBMULTCONDENSED
    TH1F* BpMonashTemplatecond = new TH1F("BpMonashTemplatecond", "B^{+}#barB^{0} Monash", bins, 0, 1);
  ...
  if (iTree == 0) { // BBMULTGENERAL
    TH1F* BpMonashRelTemplate = new TH1F("BpMonashRelTemplate", "B^{+}#Lambda_{b}^{0} / B^{+}B^{-} Junctions", bins, 0, 3);
  ...
  if (iTree == 1) { // BBMULTCONDENSED
    TH1F* BpMonashRelTemplatecond = new TH1F("BpMonashRelTemplatecond", "B^{+}#Lambda_{b}^{0} / B^{+}B^{-} Junctions", bins, 0, 3);
  ...
  for (auto& pair : hYields) {
    ...
    if(iTree == 0 && i == 0 && j == 0) { // i = 0 B-, i = 2 B0
  ...
  for (Int_t i = 0; i < vHistAndLabelsSize / Nhist; i++) {
    for (Int_t j = 0; j < 2; j++) {
  ...
  for (Int_t i = 0; i < vHistAndLabelsSize / Nhist; i++) {
    for (Int_t j = 0; j < 2; j++) {
      for (Int_t k = 0; k < Nconfigs; k++) {
	std::cout << Form("vYieldsErr[%i][%i][%i] = ", i, j, k) << vYieldsErr[i][j][k] << std::endl;
      }
    }
  }
  */

  canvasBpMonash->SaveAs("Plots/canvasBpMonashErrors.pdf");
...
int yieldSampling_BB() {

  // TFile* outputFile = new TFile("yieldErrors_TEMPORARY_bb.root", "RECREATE");
