...
// Main plotting script for doing balancing studies with the MONASH and JUNCTIONS tunes of PYTHIA 8.3
// and their dependance on event multiplicity.
...
// It is crucial that the looping over this big vector is OK, as the indices correspond to different correlations and
// binnings. I recommend playing around with the structure of these vectors by calling their histogram names.
// From experience, whenever the code shows errors, it is related to a wrong treatment of the integers or sizes of the vectors.

// Errors can be calculated with a sub-sampling procedure. This is done in a seperate script, but strongly dependent on the
// set-up of the user's computer. The idea is to repeat the calculations below on subsamples (easiest to realise by simulating
// already in subgroups and merging the output together in the final stage) and plot the results in a histogram.
// The error can be approximated by a Gaussian.
// I would not worry too much about the errors in the beginning. If an estimate is necessary, I have noticed that simulating
// 1 billion events in my standard, biased production settings (as reported in the repository) yields an error
// of about 0.1 - 1 %, depending on the extremity of the binning.

// It is straightforward to extent these studies to other dependencies (pT, rapidity, pseudorapidity) or for other sectors (strange)
// or even other simulation settings or tunes (other CR modes, ROPES).
// This can be done in a way analogously to how multiplicity is treated here, though it will require to implement this also for
// the calculation of error in the subsampling procedure.
// Most of this has been done to some extent in previous versions before the focus was switched to multiplicity studies.
// These archived versions can be found in the 'Other' repository on Github.
...
void CalculateOS_minus_SS(std::vector<HistAndLabels> vHistAndLabels,
  ...
  if (!inputTree) { std::cout << "ERROR: Can't find tree" << std::endl; }
  ...
  // The following part is kept commented, it is useful for debugging of the error values.
  /*
  std::cout << std::endl;
  ...
  */
  ...
  // If no errors are known, then this can be used as a dummy to make the scripts work even with errors enabled, but in that
  // case, take care that all dimensions make sense.
  // 1. vYieldsErr.txt
  ...
  */  
  ...
  // Calculation loop
  // This is the main loop where the OS - SS operation is performed
  // The resulting histograms are integrated over the full 2pi range and their results ('yields') are saved in a yield array.
  // This yield array is then used to plot the different histograms that we are interested in.
...
int B_Balancing_GeneralPlotting() {
  ...
  // The struct definition is shown below, for convenience.
  //
  /*
    struct DrawOptions {
    // Name which is used for writing the plots to the local directory
  ...
  */
  ...
  Bool_t writeGlobalYields = false;    // Write global yield plot to local 
  ...
  // This part of the code needs some attention and is prone to not work 'out of the box'.
  // Following how the canvasses are plotted it is not too difficult to get the individual plots out of them,
  // but is encouraged to always plot the 'combined plot' and take the subcanvas out to inspect it alone.
  Bool_t GLOBAL_CANVAS = false;
  Bool_t RELATIVE_CANVAS = true;
  ...
  // CalculateOS_minus_SS(vInput, "integrated yields for B^{+} trigger as function of pT", vHistNameAndLabels.size(), junctions, write);
  ...
  // vInput = configureInput(vPathAndLabels, vRootFileAndLabels, vHistNameAndLabels);
  // CalculateOS_minus_SS(vInput, "integrated yields for B^{+} trigger as function of multiplicity", vHistNameAndLabels.size(), write, errFileName, errTreeName);
  // }
  ...
  if (bb_MONASH_MULTIPLICITY_CONDENSED) {
    ...
    CalculateOS_minus_SS(vInput, "integrated yields for B^{+} trigger as function of multiplicity", 3*vPathAndLabels.size(), junctions, write, errFileName, errTreeName, drawOptions_bb_MONASH_MULTIPLICITY_CONDENSED);
  }
  ...
  vInput = configureInputMult(vPathAndLabels, vRootFileAndLabels, vHistNameAndLabels);
  CalculateOS_minus_SS(vInput, "integrated yields for B^{+} trigger as function of multiplicity", 3*vPathAndLabels.size(), junctions, write, errFileName, errTreeName, drawOptions_bb_JUNCTIONS_MULTIPLICITY_CONDENSED);
  }
  ...
  vInput = configureInput(vPathAndLabels, vRootFileAndLabels, vHistNameAndLabels);
  CalculateOS_minus_SS(vInput, "integrated yields for B^{+} trigger as function of multiplicity", vPathAndLabels.size()*vHistNameAndLabels.size(), junctions, write, errFileName, errTreeName, drawOptions_bb_BOTH_MULTIPLICITY_Lb);
  }

  if (bb_BOTH_MULTIPLICITY_GENERAL_LbSb) {
  vInput = configureInput(vPathAndLabels, vRootFileAndLabels, vHistNameAndLabels);
  CalculateOS_minus_SS(vInput, "integrated yields for B^{+} trigger as function of multiplicity", vPathAndLabels.size()*vHistNameAndLabels.size(), junctions, write, errFileName, errTreeName, drawOptions_bb_BOTH_MULTIPLICITY_LbSb);
  }
  ...
  vInput = configureInputMult(vPathAndLabels, vRootFileAndLabels, vHistNameAndLabels);
  CalculateOS_minus_SS(vInput, "integrated yields for B^{+} trigger as function of multiplicity", 3*vPathAndLabels.size(), junctions, write, errFileName, errTreeName, drawOptions_bb_BOTH_MULTIPLICITY_CONDENSED);
  }
  ...
  vInput = configureInputMult(vPathAndLabels, vRootFileAndLabels, vHistNameAndLabels);
  CalculateOS_minus_SS(vInput, "integrated yields for prompt D^{+} trigger as function of multiplicity", 3*vPathAndLabels.size(), junctions, write, errFileName, errTreeName, drawOptions_cc_MONASH_MULTIPLICITY_CONDENSED);
  }
  ...
  vInput = configureInputMult(vPathAndLabels, vRootFileAndLabels, vHistNameAndLabels);
  CalculateOS_minus_SS(vInput, "integrated yields for prompt D^{+} trigger as function of multiplicity", 3*vPathAndLabels.size(), junctions, write, errFileName, errTreeName, drawOptions_cc_JUNCTIONS_MULTIPLICITY_CONDENSED);
  }
  ...
  vInput = configureInput(vPathAndLabels, vRootFileAndLabels, vHistNameAndLabels);
  CalculateOS_minus_SS(vInput, "integrated yields for prompt D^{+} trigger as function of multiplicity", vHistNameAndLabels.size()*vPathAndLabels.size(), junctions, write, errFileName, errTreeName, drawOptions_cc_BOTH_MULTIPLICITY_Lc);
  }

  if (cc_BOTH_MULTIPLICITY_GENERAL_LcSc) {
  vInput = configureInput(vPathAndLabels, vRootFileAndLabels, vHistNameAndLabels);
  CalculateOS_minus_SS(vInput, "integrated yields for prompt D^{+} trigger as function of multiplicity", vHistNameAndLabels.size()*vPathAndLabels.size(), junctions, write, errFileName, errTreeName, drawOptions_cc_BOTH_MULTIPLICITY_LcSc);
  }
  ...
  vInput = configureInputMult(vPathAndLabels, vRootFileAndLabels, vHistNameAndLabels);
  CalculateOS_minus_SS(vInput, "integrated yields for prompt D^{+} trigger as function of multiplicity", 3*vPathAndLabels.size(), junctions, write, errFileName, errTreeName, drawOptions_cc_BOTH_MULTIPLICITY_CONDENSED);
  }
