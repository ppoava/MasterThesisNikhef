// SameDiffMother_Stacked_MotherStudy.C
// Documentation in DecayAndHadronisation_Stacked_MotherStudy.C

// NOTE: histograms are called "hDPhiTrSameMother", etc. in old simulations
// but are called "hDPhiOfSameMotherTr" in new simulations!
// This will be fixed once I convert everything to the new simulations, for
// now only an issue for the SameDiffMother macros.
// (see main function)
...
void readTree(vector<const char*> DPhi2DVector,
	      const char* hTrPt,
	      bool trigger,
	      bool associate,
	      bool print,
  ...
  TFile* rootFile2D = new TFile("../RootFiles/DplusDminus_600.root");
  const char* hDPhi2D;
  if (trigger) { hDPhi2D = DPhi2DVector[0]; }
  if (associate) { hDPhi2D = DPhi2DVector[1]; }
  TH2* hDPhiMother = (TH2*)rootFile2D->Get(hDPhi2D);
  TH1D* hPt = (TH1D*)rootFile2D->Get(hTrPt);
  TH1* hDPhi = hDPhiMother->ProjectionX();
  
  // I rename my variables in this way for backwards compatiblity with my code
  // The original names are kept to give insight in the meaning of the variable names
  const char* ptRegion = hDPhi2D;
  const char* ptTrRegion = hTrPt;

  // Integrate from -pi/2 to +3pi/2
  normalise(hDPhiMother, hPt, -1.57, 4.71);
  normalise(hDPhi, hPt, -1.57, 4.71); // This is DPhi between the two mothers
  ...
  //
  //
  // Now plot the trigger mother IDs ------------------------------------------------
  //
  //
  //
  //
  ...
  // hDMesonsRel->SetTitle(Form("Trigger mothers %s", ptRegion));
  hDMesonsRel->SetTitle("");
  hDMesonsRel->GetYaxis()->SetTitle("relative abundance per bin");
  hDMesonsRel->GetXaxis()->SetTitle("#Delta#phi (trigger mother, associate mother)");
  hDMesonsRel->GetYaxis()->SetRangeUser(1e-4, 2);
  // hDMesonsRel->GetXaxis()->SetRangeUser(-0.5, 0.5);
  hDMesonsRel->SetStats(0);
  hDMesonsRel->SetLineColor(kRed);
  hDMesonsRel->SetLineStyle(1);
  hDMesonsRel->SetLineWidth(3);
  hDMesonsRel->Draw("hist E");
  ...
  legendBMesons->AddEntry(hDMesonsRel, "excited d-mesons");
  ...
  hDPhi->GetXaxis()->SetTitle("#Delta#phi (trigger mother, associate mother)");
  hDPhi->SetStats(0);
  ...
  hABs0Raw->SetFillStyle(3001);
  hABs0Raw->SetFillColor(kOrange);
  hBs0Raw->SetFillStyle(1001);
  hBs0Raw->SetFillColor(kOrange);
  hAB0Raw->SetFillStyle(3001);
  hAB0Raw->SetFillColor(kOrange+4);
  hB0Raw->SetFillStyle(1001);
  hB0Raw->SetFillColor(kOrange+4);
  hBmRaw->SetFillStyle(3001);
  hBmRaw->SetFillColor(kYellow+2);
  hBpRaw->SetFillStyle(1001);
  hBpRaw->SetFillColor(kYellow+2);
  hDMesonsRaw->SetFillStyle(1001);
  hDMesonsRaw->SetFillColor(kRed);
  ...
  THStack* stackSelection = new THStack("stackSelectionTr", "");
  stackSelection->Add(hBBaryonsRaw);
  stackSelection->Add(hABs0Raw);
  stackSelection->Add(hBs0Raw);
  stackSelection->Add(hAB0Raw);
  stackSelection->Add(hB0Raw);
  stackSelection->Add(hBmRaw);
  stackSelection->Add(hBpRaw);
  stackSelection->Add(hDMesonsRaw);
  stackSelection->Draw("same hist");

  TLegend *legendStackedSelection = new TLegend(legendXmin, legendYmin, legendXmax, legendYmax);
  legendStackedSelection->SetBorderSize(0);
  legendStackedSelection->SetTextSize(0.03);
  legendStackedSelection->AddEntry(hDPhi, "#Delta#phi");
  legendStackedSelection->AddEntry(hDMesonsRaw, hDMesonsRaw->GetTitle(), "F");
  legendStackedSelection->AddEntry(hBpRaw, hBpRaw->GetTitle(), "F");
  legendStackedSelection->AddEntry(hBmRaw, hBmRaw->GetTitle(), "F");
  legendStackedSelection->AddEntry(hB0Raw, hB0Raw->GetTitle(), "F");
  legendStackedSelection->AddEntry(hAB0Raw, hAB0Raw->GetTitle(), "F");
  legendStackedSelection->AddEntry(hBs0Raw, hBs0Raw->GetTitle(), "F");
  legendStackedSelection->AddEntry(hABs0Raw, hABs0Raw->GetTitle(), "F");
  legendStackedSelection->AddEntry(hBBaryonsRaw, hBBaryonsRaw->GetTitle(), "F");
  legendStackedSelection->Draw(); 
  
  if (write) {
    cBMesons->SaveAs(Form("Plots/DecayOnly/cBMesonsTr_%s_%s.pdf", ptRegion, ptTrRegion));
    cStacked->SaveAs(Form("Plots/DecayOnly/cStackedTr_%s_%s.pdf", ptRegion, ptTrRegion));
    cStackedSelection->SaveAs(Form("Plots/DecayOnly/cStackedSelectionTr_%s_%s.pdf", ptRegion, ptTrRegion));
  }
  ...
  // hADMesonsRel->SetTitle(Form("Associate mothers %s", ptRegion));
  hADMesonsRel->SetTitle("");
  hADMesonsRel->GetYaxis()->SetTitle("relative abundance per bin");
  hADMesonsRel->GetXaxis()->SetTitle("#Delta#phi (trigger mother, associate mother)");
  hADMesonsRel->GetYaxis()->SetRangeUser(1e-4, 2);
  // hADMesonsRel->GetXaxis()->SetRangeUser(-0.5, 0.5);
  hADMesonsRel->SetStats(0);
  hADMesonsRel->SetLineColor(kRed);
  hADMesonsRel->SetLineStyle(2);
  hADMesonsRel->SetLineWidth(2);
  hADMesonsRel->Draw("hist E");
  ...
  legendBMesons->SetBorderSize(0);
  legendBMesons->SetTextSize(0.03);
  legendBMesons->AddEntry(hADMesonsRel, "excited d-mesons");
  legendBMesons->AddEntry(hBpRel, hBpRel->GetTitle());
  legendBMesons->AddEntry(hBmRel, hBmRel->GetTitle());
  legendBMesons->AddEntry(hB0Rel, hB0Rel->GetTitle());
  legendBMesons->AddEntry(hAB0Rel, hAB0Rel->GetTitle());
  ...
  THStack* stackSelection = new THStack("stackSelectionAs", "");
  stackSelection->Add(hBBaryonsRaw);
  stackSelection->Add(hABs0Raw);
  stackSelection->Add(hBs0Raw);
  stackSelection->Add(hAB0Raw);
  ...
  stackSelection->Add(hBmRaw);
  stackSelection->Add(hBpRaw);
  stackSelection->Add(hADMesonsRaw);
  stackSelection->Draw("same hist");
  ...
  legendStackedSelection->SetTextSize(0.03);
  legendStackedSelection->AddEntry(hDPhi, "#Delta#phi");
  legendStackedSelection->AddEntry(hADMesonsRaw, hADMesonsRaw->GetTitle(), "F");
  legendStackedSelection->AddEntry(hBpRaw, hBpRaw->GetTitle(), "F");
  legendStackedSelection->AddEntry(hBmRaw, hBmRaw->GetTitle(), "F");
  legendStackedSelection->AddEntry(hB0Raw, hB0Raw->GetTitle(), "F");
  legendStackedSelection->AddEntry(hAB0Raw, hAB0Raw->GetTitle(), "F");
  ...
  if (write) {
    cBMesons->SaveAs(Form("Plots/DecayOnly/cBMesonsAs_%s_%s.pdf", ptRegion, ptTrRegion));
    cStacked->SaveAs(Form("Plots/DecayOnly/cStackedAs_%s_%s.pdf", ptRegion, ptTrRegion));
    cStacked->SaveAs(Form("Plots/DecayOnly/cStackedSelectionAs_%s_%s.pdf", ptRegion, ptTrRegion));
  }
...
int SameDiffMother_Stacked_MotherStudy() {

  /*
    void readTree(const char* DPhi1D,
	      vector<const char*> DPhi2DVector,
	      const char* TrPt,
	      bool trigger,
	      bool associate,
	      bool print,
  ...
  */
  // NOTE: DPhi1D is the Delta phi between the trigger and associate mother
  // NOTE: To investigate different x-ranges, uncomment the SetXaxis line in the plotting part of the function.

  // Input for vector, 1st component is trigger, 2nd is associate
  // OLD (pre-generated ROOT files)
  vector<const char*> DPhiSameMother2DVector;
  DPhiSameMother2DVector.push_back("hDPhiTrSameMother");
  vector<const char*> DPhiDiffMother2DVector;
  DPhiDiffMother2DVector.push_back("hDPhiTrDiffMother");
  DPhiDiffMother2DVector.push_back("hDPhiAsDiffMother");

  // NEW (generated by simulation scripts found in repository)
  /*
  vector<const char*> DPhiSameMother2DVector;
  DPhiSameMother2DVector.push_back("hDPhiOfSameMotherTr");
  vector<const char*> DPhiDiffMother2DVector;
  DPhiDiffMother2DVector.push_back("hDPhiOfDiffMotherTr");
  DPhiDiffMother2DVector.push_back("hDPhiOfDiffMotherAs");
  */
  ...
  // Decay: same mother (trigger = associate)
  readTree(DPhiSameMother2DVector,
	   "hTrPtSameMother",
	   true,
	   false,
	   false,
  ...
  // Decay: different mother (trigger)
  readTree(DPhiDiffMother2DVector,
	   "hTrPtDiffMother",
	   true,
	   false,
	   false,
  ...
  // Decay: different mother (associate)
  readTree(DPhiDiffMother2DVector,
	   "hTrPtDiffMother",
	   false,
	   true,
